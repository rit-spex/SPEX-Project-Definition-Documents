...
\author{
  ...
  \IEEEauthorblockN{% This block is for author Names.
    ...
    and
    Chris Johnson\IEEEauthorrefmark{6}
  }
  \IEEEauthorblockA{% This block is for the author Affiliations, aka department and university
    ...
    \IEEEauthorrefmark{6}cxj9051@rit.edu
  }
...
Estimates put an upper bound of the time it will take for the proteins to fold at approximately fifteen to twenty seconds. If the payload is not ejected from the launch vehicle at apogee at 10,000 feet, we estimate that the payload will be in free-fall for roughly fifteen seconds from when the engine is cut off to when apogee is reached. This will likely be enough time for the proteins to fold. The period of free-fall could be extended if the payload is ejected at apogee, or by launcing to a higher altitude.
...
A payload recovery system is only necessary if the payload is ejected from the launch vehicle. Otherwise, the payload will be recovered with the launch vehicle. Recovery aids may still be encouraged to recovery time of the entire rocket system.
